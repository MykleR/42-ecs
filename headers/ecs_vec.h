...
# define ECS_VEC_MAXCAP 0x1000000000
#endif
...
#define ECS_VEC_EMPTY(vec) \
	__ECS_VEC_OUT_RANGE(vec, 0) \

#define ECS_VEC_NOT_EMPTY(vec) \
	__ECS_VEC_IN_RANGE(vec, 0) \

#define ECS_VEC_TOP(vec, cast) \
	*(cast *)(ECS_VEC_NOT_EMPTY(vec) ? __ECS_VEC_PTR(vec, (vec).len - 1) : NULL)

#define ECS_VEC_GET(vec, cast, index) \
	*(cast *)(__ECS_VEC_IN_RANGE(vec, index) ? __ECS_VEC_PTR(vec, index) : NULL)

#define ECS_VEC_CAST(vec, cast) \
	((cast *)((vec).data))

#define ECS_VEC_SET(vec, index, value) do { \
	if (__ECS_VEC_IN_RANGE(vec, index)) \
...
#define ECS_VEC_REMOVE(vec, index, ...) do {\
	if (__ECS_VEC_OUT_RANGE((vec), (index))) break; \
